...
| V-P10 | `shortcut && bidirectional` | `path.edges[{i}] — 지름길 "{from}→{to}" 가 양방향이다 — 보호대상은 앞으로만 가므로 반대 엣지는 아무도 지나지 않는다. bidirectional 을 false 로 두어라` (⚠️). ❌ 가 아닌 이유: 게임이 반대 엣지도 같이 열어 주므로(`PathGraph.OpenShortcut`) 지금 깨지지는 않는다 |
