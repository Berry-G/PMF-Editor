...
- **타임라인 그래프** (캔버스 직접 그림): x=시간. 선 1 = 모체–보호대상 거리, 막대 = 스폰 시각(종류 색), 띠 = 버스트 창·회복 창, 점 = 접촉. 적 마커(위치)는 SimResult.samples 에 기록되지 않아 M5 이후 구현 예정. 타임라인의 스폰 막대가 적 밀도를 시각화하는 데 충분하다.
...
> ✅ **해결 (2026-09-05).** 실측이 **119.067 / 56기 / 402** 로 기준값과 일치한다.
> 2026-09-04 시점의 어긋남(119.07 / 52기 / 384)은 `enterBurst`(§7-5)·묶음 종료 규칙을 게임 코드에 맞추면서 사라졌다.
> **허용 범위를 넓혀 통과시키지 마라** — 이 표가 무의미해진다. 지금은 범위 안이 아니라 정확히 일치하므로,
> 값이 흔들리면 그건 곧 회귀다.
...
- 왜 4-연결인가: 게임 아군은 가시성 그래프(`AllyWalkGraph.cs`)로 걷지만 대각선으로 도로를 "건너뛰지" 는 않는다.
  **✅ 2026-09-06 확인됨** — 씨앗 맵에서 `AllyUnit.CanReach` 와 전수 대조했다 (마을 둘 × `B` 381칸,
  Unity 에디터 `eval` 로 게임 코드 직접 호출). 불일치 0. 마을 (27,9)·(10,13) 둘 다 도달 `B` 203/381.
  같은 알고리즘이라서가 아니라 **이 맵의 도로가 축 정렬 세그먼트뿐**이라 결과가 같다 —
  대각선으로 맞닿은 도로 모서리가 생기면 가시성 그래프는 스쳐 지나가고 4-연결은 막는다.
  맵 모양이 바뀌면 다시 대조하라 (SDD-06 §2).
