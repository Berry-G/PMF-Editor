...
→ 일반 엣지(비지름길, 양 끝이 `branch` 가 아님)의 **from→to 직선(Bresenham) 위 모든 셀이 `R`** 이어야 한다. 이 한 규칙이 "코너 어긋남" 과 "도로 밖 엣지" 를 둘 다 잡는다.
...
| V-P03 | ❌ | 일반 엣지(비지름길·양 끝 비`branch`)의 Bresenham 직선 위에 `R` 아닌 셀이 있음 | §3-3 코너 규칙. 대각선이 코너를 가로지르면 적이 벽을 뚫고 걷는 것처럼 보인다 |
