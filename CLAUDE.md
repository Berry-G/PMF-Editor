...
- 배포됨: **https://pmf.manjac.co.kr** (2026-09-06). `npm run build && npm run deploy`
- 남은 것: 기획자에게 URL 전달(M6), M5 (5) 플레이 확인 → (6) 총수입 재측정
...
1. **단일 HTML 산출.** VPS 정적 호스팅(URL) + zip. 백엔드·설치형 없음 → ADR-E01
...
- **읽는 순서:** 이 파일 → `SDD-08 §0`(오해 12개) → `SDD-02`(계약) → 해당 마일스톤의 `SDD-08`·`SDD-09` 절 → 나머지는 필요할 때.
- 한 번에 마일스톤 하나 (`docs/SDD-07-로드맵.md`). DoD 를 먼저 읽어라.
- SDD 에 없는 것을 정해야 하면 **가정을 코드 주석과 완료 보고에 적어라** (`SDD-08 §14`). SDD 를 조용히 다르게 해석하지 마라. 확신이 없으면 물어라.
...
| `docs/SDD-08-확정API.md` | **구현 계약.** 시그니처·설정·파일 배치·금지 목록·보고 형식. **코드를 쓰기 전에 §0 부터 읽어라** |
| `docs/SDD-09-알고리즘.md` | 함수 본문 명세 — 파서·검증 메시지·기하·그래프·시뮬·렌더. 여기 있는 순서·동률 규칙은 결정론 때문이니 지켜라 |
| `docs/adr/` | 결정과 이유 |
