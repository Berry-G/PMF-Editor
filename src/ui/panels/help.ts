...
const GROUPS: Group[] = [
  ...
  },
  {
    ...
    rows: [
      ...
      { keys: '우클릭', desc: '지우개 — 배치 가능(B)으로 되돌린다' },
...
const NOTES: string[] = [
  '경로 도구(N · E · V)에서 캔버스를 우클릭하면 메뉴가 열린다 — 노드 위면 이름·역할·버스트 트리거·삭제, 엣지 위면 지름길·방향·삭제, 빈 칸이면 노드/시작점/탈출점 놓기. 셀 도구(B L R F)에서 우클릭은 그대로 지우개다.',
  '출발점·도착점은 따로 있는 물건이 아니라 노드의 역할이다. 시작 노드에서 보호대상과 공장(모체)이 함께 출발한다. 시작은 정확히 1개, 탈출은 1개 이상이어야 한다 (V-P02).',
  '지름길 만들기: E 로 노드 하나를 클릭 → 두 번째 노드를 Shift+클릭. allowed 가 보호대상(Escortee)으로 잠기고 단방향이 된다 (ADR-E11).',
