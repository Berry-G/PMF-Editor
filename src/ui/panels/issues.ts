 ...
 * 바꾸면 안 되는 것: 클릭 시 view 이동 + selection 갱신.
 * 근거: SDD-03 §9 [D-03-09], ADR-E08
 */
...
export function mountIssues(store: Store, container: HTMLElement, view: View): void {
  const render = (state: EditorState) => {
    ...
    for (const iss of state.issues) {
      ...
      div.onclick = () => {
        if (iss.cells && iss.cells.length > 0) { view.panX = -iss.cells[0]!.x * 32 + 100; view.panY = -iss.cells[0]!.y * 32 + 100; store.notifyViewChanged(); }
        if (iss.nodeIds && iss.nodeIds.length > 0) store.update((_s) => ({ selection: { kind: 'nodes' as const, ids: iss.nodeIds! } }));
        else if (iss.edgeIndex !== undefined) store.update((_s) => ({ selection: { kind: 'edge' as const, index: iss.edgeIndex as number } }));
      };
